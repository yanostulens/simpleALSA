...
static int write_and_poll_loop(snd_pcm_t *handle, signed short *samples) {
  ...
  ufds[0].fd =
      pipe_fds[0]; // store read end of pipe //TODO make pipe to signal end
  ufds[0].events = POLLIN;

  if ((err = snd_pcm_poll_descriptors(handle, ufds + 1, count - 1)) <
      0) { // dont give ALSA the first poll descriptor
...
int startAlsa() {
  ...
  sizeof(snd_pcm_channel_area_t)); if (areas == NULL) { printf("No enough
  memory\n"); exit(EXIT_FAILURE);
  }
